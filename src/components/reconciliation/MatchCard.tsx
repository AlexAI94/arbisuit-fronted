...
import {
  CheckCircle2, XCircle, ArrowRight, ArrowDownLeft, ArrowUpRight,
  Zap, Brain, GitMerge, User, Loader2, ChevronDown, ChevronUp
} from "lucide-react";
...
export function MatchCard({ item, onApproved, onRejected }: MatchCardProps) {
  ...
  const handleApprove = async () => {
    ...
    try {
      await api.post(`/reconciliations/items/${item.id}/approve`, { item_id: item.id });
      onApproved();
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      await api.delete(`/reconciliations/items/${item.id}/reject`, {
  ...
  return (
    <div className={cn(
      ...
      <div className="px-4 py-3 flex items-center gap-3">
        ...
        {needsApproval && (
          <span className="text-xs text-arbi-yellow flex-shrink-0 font-medium">
            Revisar
          </span>
        )}
      ...
      {/* Match visual: banco ↔ cripto */}
      <div className="px-4 pb-3 grid grid-cols-[1fr_auto_1fr] gap-3 items-center">
        {/* Lado bancario */}
        <div className="bg-[#0F1117] rounded-lg p-3 space-y-1">
          <div className="flex items-center gap-1.5 mb-1.5">
            {item.bank_direction === "credit"
              ? <ArrowDownLeft className="w-3.5 h-3.5 text-arbi-green" />
              : <ArrowUpRight className="w-3.5 h-3.5 text-arbi-red" />}
            <span className="text-xs text-muted-foreground uppercase tracking-wide">Banco</span>
          </div>
          <p className={cn(
            ...
            {item.bank_direction === "credit" ? "+" : "-"}
            {formatCurrency(item.bank_amount || 0)}
          </p>
          ...
          {item.bank_counterpart && (
            <p className="text-xs text-muted-foreground truncate">{item.bank_counterpart}</p>
          )}
        </div>
      ...
      {/* Detalle expandido */}
      {expanded && (
        <div className="px-4 pb-3 border-t border-[#1E2534] pt-3 space-y-2 animate-fade-in">
          {item.notes && (
            <p className="text-xs text-muted-foreground bg-[#0F1117] rounded-lg px-3 py-2">
              {item.notes}
            </p>
          )}
          <div className="grid grid-cols-3 gap-3 text-xs">
            <div>
              <p className="text-muted-foreground">Δ Monto</p>
              <p className="text-foreground font-mono">{item.amount_diff_pct?.toFixed(2) || "0"}%</p>
            </div>
            <div>
              <p className="text-muted-foreground">Tipo match</p>
              <p className="text-foreground capitalize">{item.match_type.replace("_", " ")}</p>
            </div>
            <div>
              <p className="text-muted-foreground">Confirmado por</p>
              <p className="text-foreground capitalize">{item.matched_by === "engine" ? "Motor" : "Usuario"}</p>
            </div>
          </div>
        </div>
      )}
